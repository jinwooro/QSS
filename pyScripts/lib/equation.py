...
def reformCoupledQss(expr, Xq):
    ...
    for key, val in Xq.items():
        newExpr1 = newExpr1.subs(S.sympify(key + '(t)'), val[0])
        newExpr2 = newExpr2.subs(S.sympify(key + '(t)'), val[1])
...
class GUARD:
    ...
    def computeDelta(self, Xq, Iq):
        ...
        if (lqss.is_number) and (hqss.is_number):
            return math.inf
        iter = 0
        while(iter <= self.iteration):
            ...
            if (len(tl) == 0) or (len(th) == 0):
                t1 = S.solve(lqss - hqss - self.vtol)
                t2 = S.solve(hqss - lqss - self.vtol)
                # return min(tl + th)
                return min(t1 + t2)
            t1 = min(tl)
            t2 = min(th)
